...
export function composeBatch(prompt:string,count:number,options:{genre?:string;energy?:number;seed?:number}={}):Song[]{
  ...
  return Array.from({length:count},(_,i)=>compose(prompt,{...options,seed:(base+Math.imul(i,0x9e3779b9))>>>0}));
}
