...
export function moreLike(source:Song,count:number,seed:number):Song[]{
  ...
  return Array.from({length:count},(_,i)=>{
    ...
    if(source.arrangement){const scored=buildScore(result,source.bars,source.arrangement.plan);scored.arrangement!.bars[0]=structuredClone(source.arrangement.bars[0]);return validateSong(applyExclusions(scored));}
...
export function remix(s:Song,seed:number){
  ...
  delete remixed.arrangement;
  return s.arrangement?validateSong(buildScore(remixed,s.bars,s.arrangement.plan)):remixed;
}
